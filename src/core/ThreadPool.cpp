...
#define THREADPOOL_PROCESS_DEBUG(MESSAGE)                                                                              \
    THREADPOOL_DEBUG("Threadpool process PID = " << std::this_thread::get_id() << ": " << MESSAGE)

namespace Afina {
namespace Core {

ThreadPool::ThreadPool()
    : _low_watermark(0), _hight_watermark(0), _max_queue_size(0), _idle_time(0), state(ThreadPool::State::kStopped),
      _count_free_threads(0) {}
    ...
    try {
        {
            ...
            std::cout << "My ID: " << this_id << std::endl;
        }
...
void ThreadPool::_ExecuteTasks() {
    ...
    while (!tasks.empty()) {
        ...
        try {       // Other problems are system problems in thread and it should be finished - so try-catch is in
                    // _ThreadFunction()
            task(); // Execute
        } catch (std::exception &exc) {
...
bool ThreadPool::_TryUnregisterThread() {
    std::unique_lock<std::mutex> __lock(threadpool_mutex);
    if (threads.size() <= _low_watermark + 1 && state.load() == ThreadPool::State::kRun) {
        return false;
    } // Cannot finish due to low watermark
...
void ThreadPool::Stop(bool await) {
    THREADPOOL_DEBUG(__PRETTY_FUNCTION__);
    if (state.load() != ThreadPool::State::kRun) {
        return;
    }
    ...
    if (await) {
        while (!threads.empty()) {
            _ThreadsIterator next_thread;
            {
                ...
                if (next_thread == threads.end()) {
                    break;
                } // All threads was finished before we get unique lock
            }     // Free lock to get a allow thread to finish

            try {
                if (next_thread->second.joinable()) {
                    next_thread->second.join();
                }
            } catch (std::system_error &exc) {
                if (exc.code() == std::errc::no_such_process) {
                    continue;
                } // The process has finished before we've called join()
            }
        }
